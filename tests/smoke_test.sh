...
echo ""
echo "[STT explicit language]"
if [ -f "$TMPDIR/tts.mp3" ] && [ "$STATUS" = "200" ]; then
    BODY=$(curl -s -H "Authorization: Bearer $API_KEY" -F "file=@$TMPDIR/tts.mp3;filename=test.mp3" -F "language=en" "$BASE_URL/v1/audio/transcriptions")
    if echo "$BODY" | python3 -c "import sys,json; t=json.load(sys.stdin).get('text',''); exit(0 if len(t)>5 else 1)" 2>/dev/null; then
        TEXT=$(echo "$BODY" | python3 -c "import sys,json; print(json.load(sys.stdin)['text'])")
        pass "STT (language=en) → \"$TEXT\""
    else
        fail "STT (language=en)" "$BODY"
    fi
else
